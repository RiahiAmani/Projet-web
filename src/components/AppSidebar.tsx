...
const publicLinks = [
  { to: "/", icon: Globe, label: "Accueil public" },
  { to: "/recherche", icon: Search, label: "Publications" },
...
export default function AppSidebar() {
  ...
  const isPublic = !location.pathname.startsWith("/admin") && !location.pathname.startsWith("/moderateur");

  const links = isPublic ? publicLinks : isModerateur ? modLinks : adminLinks;
  const roleLabel = isPublic ? "Utilisateur" : isModerateur ? "Modérateur" : "Administrateur";
        ...
        <div className="flex gap-1 flex-wrap">
          <NavLink
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
              location.pathname.startsWith("/admin") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-muted hover:text-sidebar-foreground"
          ...
          <NavLink
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
              isPublic ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-muted hover:text-sidebar-foreground"
            } ${collapsed ? "justify-center" : ""}`}
          >
            <Globe size={14} />
            {!collapsed && "Public"}
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto scrollbar-thin">
        ...
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
            end={link.to === "/admin" || link.to === "/moderateur" || link.to === "/"}
            className={({ isActive }) =>
